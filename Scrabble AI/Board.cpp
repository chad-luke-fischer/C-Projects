...
void Board::initBoard(std::string init_board)
{
	int rows = getRows();
	int cols = getColumns();
	std::ifstream ifile(init_board);
	std::string tline; //temp line

	std::vector<Tile*> existing_tiles;

	for(int i = 0; i < rows; i++)
	{
		tline.clear();
		std::getline(ifile, tline);
		for(int j = 0; j < cols; j++)
		{
			...
			if(tStr != "...")
			{
				int score = ((int(tStr[1])-48)*10) + (int(tStr[2])-48);
				_squares[i][j]->placeTile(new Tile(tStr[0], score));
			}
...
Square* Board::getSquareM(int x, int y)
{
