...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		if(configFile->has_init)
		{
			board->initBoard(configFile->init_board);
		}
...
// 	Super_AI* CPU = new Super_AI("CPU",configFile->handSize, board, dictionary);
// 	CPU->anchor_down();

// 	ConsolePrinter::printBoard(*board);

// 	CPU->addTiles(bag->drawTiles(configFile->handSize));

// 	ConsolePrinter::printHand(*CPU);

// 	for(int i = 1; i <= board->getRows();i++)
// 	{
// 		for(int j = 1; j <= board->getColumns(); j++)
// 		{
// 			if(board->getSquareM(i,j)->isAnchor())
// 			{
// 				std::cout << "0";
// 			}
// 			else
...
// }
	...
	for(size_t playerNum = 1; playerNum <= numPlayers; ++playerNum)
	{
		...
		//account for AI name inputs
		std::string possible_AI = playerName.substr(0,3);
		if(possible_AI == "CPU" || possible_AI == "cpu")
		{
			std::cout << "CPU Player " << playerNum << ", named \"" << playerName << "\", has been added." << std::endl;
			players.push_back(new Super_AI(playerName, configFile->handSize, board, dictionary));
		}
		else
		{
		...
		switch(state)
		{
			...
			case ParserState::IN_VALUE:
				if(currChar == '\n' || currChar == ' ')
				{
					...
					else if(keyBuffer == "INIT") //added this
					{
						init_board = valueBuffer;
						has_init = true;
					}
					else
					{
